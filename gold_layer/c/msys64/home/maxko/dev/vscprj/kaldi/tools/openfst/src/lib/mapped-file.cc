...
#include <fcntl.h>
#if defined(_WIN32)
  // В Windows mman не нужен для базовой работы OpenFST в Kaldi
#else
  #include <sys/mman.h>
#endif
#include <sys/types.h>
...
MappedFile::~MappedFile() {
  // В Windows мы не выделяли память через mmap, поэтому здесь пусто
}

MappedFile *MappedFile::Map(std::istream *istrm, bool memorymap,
                           const std::string &source, size_t pos) {
  // В Windows принудительно отключаем memorymap и возвращаем nullptr, 
  // чтобы FST читал файл обычным способом (через поток)
  return nullptr; 
}

MappedFile *MappedFile::MapFromFileDescriptor(int fd, int mode, size_t pos) {
  return nullptr;
}
